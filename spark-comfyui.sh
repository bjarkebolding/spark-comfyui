...
# neither build nor run environment — so it lives in container/Dockerfile now,
# where the pip calls actually are.)
...
# This script sources nothing from mods/ or container/, by design as of
# 2026-08-08. It used to source mods/_lib/mod_common.sh, but its only real
# use of that library was sage_fallback_counts (below), a grep over a log
# file. Everything else in there needs the venv's python with torch,
# sageattention and onnxruntime importable, none of which exists on the host,
# so the host could never have called any of it. Those live in
# container/gb10.sh now and are sourced inside the image.
#
...
# attention instead (comfy/ldm/modules/attention.py), so the launch-time
# kernel gate cannot see these. Prints "<total> <benign>", where benign =
...
# failing), up to ~18x slower sampling. Both greps end '|| true': grep exits
# 1 when it matches nothing, which under 'set -euo pipefail' would take
# doctor down on a perfectly healthy log.
...
#   <last gen s|-> <in-flight s|-> <ok|err|cancel|-> <finished HH:MM:SS|->
...
cmd_backup() {
  ...
  # ${stage:-} not $stage: this trap can fire while the function scope is
  # unwinding from a set -e exit, and an unset local would make the trap
  # itself die under set -u, masking the real error with a bash message.
...
cmd_restore() {
  ...
  # itself die under set -u, masking the real error with a bash message.
  trap 'rm -rf "${stage:-}"' EXIT
  # Explicit, because set -e would otherwise abort inside tar before the
  # format check below could produce its much clearer message. Anything that
  # is not a readable gzip tar fails here with the file named.
  tar -xzf "$archive" -C "$stage" 2>/dev/null \
    || die "not a readable .tgz archive: $archive
(a spark-comfyui backup is gzipped tar, as written by '$0 backup')"
...
# The host<->image contract, defined ONCE and consumed twice: by doctor's live
# gate block and by _image_contract below. Keeping two copies of this list is
# how the original bug would come back — doctor could be changed to look in one
# place while the checker still tolerated two, and the checker would pass while
# doctor broke.
#
# GATE_LIBS is ordered: newest layout first, older fallbacks after. gb10.sh
# since 2026-08-08; mods/_lib/mod_common.sh is where these lived before that,
# and images built then are still valid rollback targets.
readonly GATE_LIBS="/opt/spark/gb10.sh /opt/spark/mods/_lib/mod_common.sh"
readonly GATE_FUNCS="sage_kernel_ok onnx_gpu_ok kitchen_nvfp4_ok torch_cuda_diag"

# The host<->image contract.
#
# The host reaches INTO the image in a handful of fixed places, and until
# 2026-08-09 nothing declared that interface or checked it. That is not
# theoretical: renaming the gate library on the host side (mod_common.sh ->
# gb10.sh) silently broke doctor for every image built before the rename, and
# the break only surfaces after 'update --rollback' — the exact moment a user
# is already worried and needs doctor to be trustworthy. It was found by
# reading the diff, not by any gate, because every gate we had only ever
# exercised the image it had just built.
#
# So this checks the INTERFACE, not the hardware: no GPU, no model, a couple of
# seconds. It answers one question about any image, including old ones: "if the
# user rolled back to this, would this host script still work with it?"
#
# Not checked on purpose: the ComfyUI-Manager model-list.json that 'recipe
# capture' reads, because that path already degrades gracefully to an empty
# catalogue, and /opt/spark/comfyui-nodes.list, because it is a mount target
# docker creates rather than something the image must provide.
_image_contract() {
  # -i is load-bearing: `bash -s` reads the script from stdin, and without -i
  # docker attaches no stdin, so bash gets an EMPTY script and exits 0. That
  # turns this into a gate that passes everything, which is worse than having
  # no gate at all. Caught only because the negative tests were re-run after a
  # refactor; the positive tests were perfectly happy.
  docker run --rm -i --entrypoint bash "$1" -s -- "$GATE_LIBS" "$GATE_FUNCS" <<'CONTRACT'
    fail=0
    say() { printf "    %s\n" "$*"; }
    [[ -x /opt/spark/entrypoint.sh ]]     || { say "no executable /opt/spark/entrypoint.sh (container cannot start)"; fail=1; }
    [[ -r /opt/ComfyUI/main.py ]]         || { say "no /opt/ComfyUI/main.py (mounts land under /opt/ComfyUI)"; fail=1; }
    [[ -x /opt/comfyui-env/bin/python ]]  || { say "no venv python at /opt/comfyui-env/bin/python"; fail=1; }
    lib=""
    for c in $1; do
      [[ -r "$c" ]] && { lib="$c"; break; }
    done
    if [[ -z "$lib" ]]; then
      say "no gate library (neither gb10.sh nor mods/_lib/mod_common.sh) — doctor cannot run its gates"
      fail=1
    else
      # Sourcing it is the real test. The library is a fragment that assumes
      # the caller provides these, exactly as doctor and the entrypoint do.
      log() { :; }; warn() { :; }; die() { :; }; info() { :; }
      # shellcheck disable=SC1090
      source "$lib" 2>/dev/null || { say "gate library $lib failed to source"; fail=1; }
      for f in $2; do
        declare -F "$f" >/dev/null 2>&1 || { say "gate function $f missing from $lib"; fail=1; }
      done
    fi
    exit "$fail"
CONTRACT
...
cmd_doctor() {
  ...
  (that ignores the pin; if it passes doctor and a generation, move
   TORCH_VERSION in container/Dockerfile)"
    ...
    # Every image you could still land on must work with THIS host script.
    # :latest is covered by the live gates below; these are the ones nothing
    # else ever exercises, which is precisely why they broke unnoticed once.
    local rb rb_out
    for rb in previous $(docker images "$CONTAINER_IMAGE" --format '{{.Tag}}' 2>/dev/null \
                           | grep '^keep-' | sort -u || true); do
      docker image inspect "$CONTAINER_IMAGE:$rb" >/dev/null 2>&1 || continue
      if rb_out="$(_image_contract "$CONTAINER_IMAGE:$rb")"; then
        ok "rollback target :$rb still satisfies the host contract"
      else
        bad "rollback target :$rb does NOT work with this version of the tool —
  'update --rollback' onto it would leave doctor unable to check it:"
        printf '%s\n' "$rb_out"
      fi
    done
  ...
  local gate_rc=0
  docker run --rm -i --gpus all --entrypoint bash "$CONTAINER_IMAGE:latest" \
    -s -- "$GATE_LIBS" <<'EOS' || gate_rc=$?
set -uo pipefail
ok()   { printf '  \033[1;32m[PASS]\033[0m %s\n' "$*"; }
bad()  { printf '  \033[1;31m[FAIL]\033[0m %s\n' "$*"; fails=$((fails+1)); }
log()  { printf '\n\033[1m== %s ==\033[0m\n' "$*"; }
fails=0
# The gate library moved from mods/_lib/mod_common.sh to gb10.sh on
# 2026-08-08, and THIS BLOCK RUNS INSIDE WHATEVER IMAGE IS TAGGED :latest,
# which after 'update --rollback' is an older one that has only the old path.
# Sourcing the new name unconditionally left every gate undefined and made
# doctor report a healthy rolled-back image as broken — a false alarm at the
# exact moment the user is already worried (golden rule 5). Try both.
gate_lib=""
for c in $1; do [[ -r "$c" ]] && { gate_lib="$c"; break; }; done
[[ -n "$gate_lib" ]] || { echo "  no gate library in this image (looked in: $1)"; exit 1; }
source "$gate_lib"
