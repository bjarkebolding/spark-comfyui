...
_recipe_fetch() {
  ...
  # the only way to show the whole job as well as the current file. So wget is
  # unconditionally -q and the rendering below is what differs by tty.
  ...
  wget -c --tries=5 --waitretry=5 --timeout=30 --read-timeout=120 \
    -q "${auth[@]}" -O "$part" "$url" >/dev/null 2>"$errf" &
  wpid=$!
  if [[ -t 1 ]]; then
...
# $1 done  $2 total  $3 elapsed_s  $4 all_done  $5 all_total
_recipe_render() {
  awk -v d="$1" -v t="$2" -v e="$3" -v ad="$4" -v at="$5" -v w=28 '
    function human(b) {
