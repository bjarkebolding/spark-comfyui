...
entrypoint's torch guard (ensure_torch_cuda) already verifies and repairs
torch before every launch via real torch.cuda.is_available() checks, not
string parsing. Belt-and-suspenders via pip_auto_fix.list isn't needed and
actively hurts here.

Invoked by container/entrypoint.sh as `manager-config.py apply`: write the
config, print a short status, exit 0.

Lived at mods/30-manager-config/configure.py until 2026-08-08. It was never a
mod: no run.sh, no mod contract, and the entrypoint always called it directly.
It is an entrypoint helper, so it sits beside the entrypoint now, the same way
build-patches.sh sits beside the build.

One mode. A `verify` mode (exit 0 if network_mode was personal_cloud) went in
the same change: nothing ever called it. The one place that reports this
setting is `status`, which reads the file on the host, and the entrypoint
asserts the config rather than testing it.
...
if mode != "apply":
    sys.exit("manager-config.py: unknown mode %r (the only mode is 'apply')" % mode)
...
# entrypoint's ensure_torch_cuda guard are the real protection; nothing
# replaces this file.
